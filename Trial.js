// multiDrilldown.js (Search presets inside w2ui toolbars)
// Features:
...
// - Preset picker is placed INSIDE each w2ui grid's toolbar
...
export class MultiDrilldown {
  ...
  async _renderStep(state, replaceForward = false, noWire = false) {
    ...
    // --- NEW: install preset picker INSIDE POPUP grid toolbar
  ...
  _ensureToolbar(grid) {
    ...
    // If grid already rendered, refresh to show toolbar
    try { grid.refresh?.(); } catch {}
  }
  ...
  _installToolbarPreset(grid, presets, selectId) {
    ...
    // Insert a small spacer + our HTML select
    try {
      grid.toolbar.add([
        { type: 'break', id: 'mdspacer' },
        {
          ...
          id: 'mdpreset',
          html: () => {
            // build options dynamically each render to reflect current presets
      ...
      // Bind change handler after each toolbar refresh
      const bind = () => {
      ...
      // Bind right away (after DOM paints), and also on grid refreshes
  ...
  _applyPresetList(grid, presets, autoApplyFirst = true) {
    ...
    if (Array.isArray(presets) && presets.length && autoApplyFirst) {
      ...
      // set select index to 0 if present
