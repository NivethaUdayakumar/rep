// multiDrilldown.js (Search presets inside w2ui toolbars + Promote data files w/ eN tokens)
// Features:
// - Tokenized drilldown (a*/b*/c*/...)
// - Breadcrumbs + Back/Forward
// - HTML or CSV steps
// - Promote button (main table only) -> POST /api/promote
// - Promote payload now includes resolved files f1,f2,... from promoteDataFiles using eN tokens
// - Right-click copy to clipboard (main + popup)
// - Default search presets for Main + per (triggering column, popup level)
// - Preset picker placed INSIDE each w2ui grid's toolbar
...
export class MultiDrilldown {
  constructor({
    ...
    promoteCheck = [],       // [colIdx,...] build key from these
    promoteData = [],        // [colIdx,...] store header->value in JSON
    promoteDataFiles = [],   // [ 'e1_e2_e0.html', 'logs/e0.txt', ... ]
    sep = '_',
    debug = true,
    defaultSearches = {
  ...
  async init() {
    ...
    // Preset picker INSIDE MAIN grid toolbar
    this._ensureToolbar(grid);
    this._installToolbarPreset(grid, this.defaultSearches?.main || [], `${grid.name}-preset`);
    this._applyPresetList(grid, this.defaultSearches?.main || [], /*autoApplyFirst=*/true);
  ...
  _setupPromoteOnMainGrid(grid) {
    ...
    col.render = (rec) => {
      // only for parent rows (not children & not summaries)
  ...
  async _handlePromoteClick(grid, recid) {
    ...
    // Build key from promoteCheck indices (e.g., "col0_col1")
    ...
    // Collect promoteData fields as { header: value }
    const bodyObj = {};
    ...
    // Resolve promoteDataFiles -> f1, f2, ... using eN tokens (0-based)
    ...
    // NEW payload shape: { "<key>": { ... } }  (no "key", no "data")
  ...
  // Replace eN tokens with row values from the MAIN grid row being promoted (0-based indices)
  ...
  async _renderStep(state, replaceForward = false, noWire = false) {
    ...
    // Preset picker INSIDE POPUP grid toolbar
    ...
    this._applyPresetList(dd, presets, /*autoApplyFirst=*/true);
  ...
  /* ======================= TOKEN RESOLUTION (drilldown patterns) ======================= */
  _resolvePattern(pattern, contexts) {
    const mapLetterToIndex = (ch) => ch.toLowerCase().charCodeAt(0) - 97; // 'a'->0, 'b'->1, ...
    const safePart = (s) =>
      String(s ?? '').replace(/[^\w\-./]+/g, '_').replace(/^_+|_+$/g, '');
