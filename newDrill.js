// multiDrilldown.js (NEW API + Search Presets)
// Features:
...
// - Default search presets for Main + per (triggering column, popup level) with a small picker
...
export class MultiDrilldown {
  constructor({
    ...
    debug = true,
    defaultSearches = {
      // main: [ { text, data:[{field,type,operator,value(,value2)?}], logic:'AND'|'OR' }, ... ],
      // byCol: { [colIdx]: { [levelStepIdx]: [ {text,data,logic}, ... ] } }
    }
  } = {}) {
    ...
    // Default searches config
    this.defaultSearches = defaultSearches || {};
    // runtime
    this._openOnce = false;
    this._stack = [];   // [{ step, patterns, contexts, title, triggerCol }]
    this._idx = -1;
    this._level0Fields = [];
  }
  ...
  async init() {
    ...
    // Promote button only on parent rows
    ...
    // --- NEW: Search presets on MAIN ---
    this._installMainPresetPicker(grid, this.defaultSearches?.main || []);
    ...
    grid.on('click', async (ev) => {
      ...
      await this._openSequence({ patterns, contexts: [mainCtx], title, triggerCol: col });
    });
  }
  ...
  _setupPromoteOnMainGrid(grid) {
    ...
    col.render = (rec) => {
      // show only for parent rows (not children & not summaries)
  ...
  /* ======================= SEQUENCE NAV ======================= */
  async _openSequence({ patterns, contexts, title, triggerCol }) {
    await this._ensurePopup(title, this._crumbsFor({ step: 0, patterns, contexts }));
    await this._renderStep({ step: 0, patterns, contexts, title, triggerCol }, false);
    this._stack = [{ step: 0, patterns, contexts, title, triggerCol }];
    this._idx = 0;
  ...
  async _renderStep(state, replaceForward = false, noWire = false) {
    ...
    // --- NEW: install picker + apply popup presets
    const presets = this._searchPresetsFor(state);
    this._installPopupPresetPicker(presets);
    this._applyPresetList(w2ui.ddGrid, presets, /*autoApplyFirst=*/true);
    ...
    const shell = `
      <div id="dd-wrap" style="padding:8px; display:flex; flex-direction:column; gap:8px; height:100%; box-sizing:border-box;">
        <div id="dd-bar" style="display:flex; align-items:center; gap:8px; flex:0 0 auto;">
          ...
          <label style="margin:0 6px 0 0;">Preset:</label>
          <select id="dd-preset" class="w2ui-input" style="max-width:280px;"></select>
        </div>
  ...
  /* ======================= Search Presets (Main) ======================= */
  _installMainPresetPicker(grid, presets) {
    // make a slim bar above the grid box
    try {
      const box = grid.box;
      if (!box || box._md_preset_installed) return;
      ...
      bar.innerHTML = `
        ...
        <select class="w2ui-input" id="${grid.name}-preset" style="max-width:320px;"></select>
      `;
      box.parentNode.insertBefore(bar, box);
      box._md_preset_installed = true;

      const sel = bar.querySelector('select');
      this._fillPresetOptions(sel, presets);
      sel.addEventListener('change', () => {
        const p = this._selectedPresetFrom(sel, presets);
        this._applyPreset(grid, p);
      });

      // auto-apply first if any
      if (presets?.length) {
        sel.selectedIndex = 0;
        this._applyPreset(grid, presets[0]);
      }
    } catch (e) {
      console.warn('Failed to install main preset picker:', e);
    }
  }

  /* ======================= Search Presets (Popup) ======================= */
  _installPopupPresetPicker(presets) {
    const sel = document.getElementById('dd-preset');
    if (!sel) return;
    this._fillPresetOptions(sel, presets);
    sel.onchange = () => {
      const p = this._selectedPresetFrom(sel, presets);
      this._applyPreset(w2ui.ddGrid, p);
    };
  }

  _fillPresetOptions(selectEl, presets) {
    if (!selectEl) return;
    selectEl.innerHTML = '';
    const opts = presets && presets.length ? presets : [];
    if (!opts.length) {
    ...
    opts.forEach((p, i) => {
      const o = document.createElement('option');
      o.value = String(i);
      o.textContent = String(p.text ?? `Preset ${i+1}`);
      selectEl.appendChild(o);
    });
  }

  _selectedPresetFrom(selectEl, presets) {
    const idx = Number(selectEl?.value ?? -1);
    if (Number.isNaN(idx) || idx < 0) return null;
    return (presets && presets[idx]) || null;
  }
  ...
  _applyPreset(grid, preset) {
    ...
    for (const d of preset.data) {
      // d: { field, type, operator, value(, value2)? }
      const field = d.field;
      if (!field) continue;
    ...
    try {
      ...
      // optionally keep search panel open
      // grid.searchOpen?.();
    } catch (e) {
